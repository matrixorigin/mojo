...
%% article.
\begin{abstract}
The rapid advancement of artificial intelligence has made data the cornerstone 
of modern software systems. AI systems require vast amounts of high-quality data 
for training, fine-tuning, and operation, making data engineering a critical discipline. 
As data projects become increasingly complex and dynamic, with continuous 
modification, experimentation, and collaboration, version control for data 
has become not just convenient but necessary. 

We present a version control system for data implemented in MatrixOne, 
a relational database system. This system enables git-like operations 
on database tables, allowing users to branch and fork tables, 
compute diffs and merge changes between branches, and review, revert, 
or bisect changes on large datasets almost instantly, 
without risk of data loss or corruption.

This version control system for data brings the power and convenience 
of version control to data engineering, just as source code version control 
systems have transformed software engineering. Teams of data engineers 
can collaborate on shared datasets using familiar workflows: creating branches, 
submitting pull requests, conducting reviews, and running CI/CD pipelines, 
all efficiently and safely. Changes in the development environment are 
isolated from production in both data integrity and computing resources. 
Verified changes can be published to production in a single transaction, 
...
Conflicts and resolutions of the conflicts between snapshot $T_{sn2}$ and 
snapshot $TClone_{sn3}$
are computed differently depending on whether table \texttt{T} and \texttt{TClone} 
have primary keys.
First, let us consider the case that \texttt{T} and \texttt{TClone} have 
the same primary key on column \texttt{a} and that the \texttt{SNAPSHOT DIFF}
query has found a potential conflict on a row identified by \texttt{a\_value}.  
The potential conflict may be due to one of the following scenarios:
...
This means that only $TClone_{sn3}$ has inserted the row with primary key 
...
for each row that is found by \texttt{SNAPSHOT DIFF}, there may 
be multiple rows with the same values in all columns in the 
common base revision $T_{sn1}$, the snapshot $T_{sn2}$, and the snapshot $TClone_{sn3}$.  
We use $N_{sn1}$, $N_{sn2}$, and $N_{sn3}$ to denote the number of 
such rows of the same values.  Let $\delta_T = N_{sn2} - N_{sn1}$ 
and $\delta_{TClone} = N_{sn3} - N_{sn1}$, we need to consider the 
following cases:
\begin{enumerate}
...
case to be a true conflict. \texttt{SKIP} mode
will keep $N_{sn2}$ rows, \texttt{ACCEPT} mode will keep $N_{sn3}$ rows
, and \texttt{FAIL} mode will cause the merge to fail.
\end{enumerate}
In case 3, it is possible to let \texttt{FAIL} mode 
take a more relaxed resolution on conflicts. For example, sometimes it is
reasonable to accept $max(N_{sn2}, N_{sn3})$ rows in the final merge result.  
MatrixOne decided to treat case 3 as a true conflict to avoid 
...
cancelled out by the aggregation.  
With the diff aggregation result, \texttt{SNAPSHOT DIFF} will return a result 
by, 
\begin{enumerate}
...
as described in Section \ref{sec:diff}.  The plus and minus sign in the diff 
aggregation tells whether a row is inserted to a snapshot or deleted from the
common base revision.  In the case that \texttt{T} has a primary key, we can 
use this information to tell whether a potential conflict is a true conflict 
(if this primary key appears in both $\Delta_{sn2}$ and $\Delta_{sn3}$)
or a false conflict (if this primary key appears in only one 
of $\Delta_{sn2}$ and $\Delta_{sn3}$).
...
have treated this as a true conflict and would have kept the row from $T_{sn2}$.  
...
deleted row from the common base revision. In practice, potential conflicts caused 
by this "move" case is rare and we seldom need to lookup the full deleted row from 
the common base revision.

When table \texttt{T} does not have a primary key, we perform the same 
scan of $\Delta_{sn2}$ and $\Delta_{sn3}$ and the same diff aggregation.
...
In this case, merge will need to read out the all columns of the deleted row 
from the common base revision using physical rowid.  Merge will follow the
conflict resolution strategy of Section \ref{sec:vcop}, on on the result of
the diff aggregation.

\subsection{Two Way Merge}
Uses do not need to specify the common base revision in the optiolnal 
\texttt{BASED ON} clause of the \texttt{SNAPSHOT MERGE} statement.  
In most cases, MatrixOne does proper bookkeeping of snapshot and clone 
lineages so that the system may be able to find the common base revision of \texttt{T} 
...
or on \texttt{TClone} between \texttt{sn1} and \texttt{sn3}.  
MatrixOne performs
...
using the diff aggregation described in Section \ref{sec:threewaymerge}.
...
by letting user specify a \texttt{BASED ON} clause in the \texttt{SNAPSHOT DIFF}
statment. In fact, the plus and minus sign in the diff aggregation is exactly 
the information needed to compute three-way diff.  While a general three-way diff 
is useful in understanding potential conflicts of complex history, we believe 
end users, when performing tasks such as data change review, will most likely 
work with the two-way diff result.   Thus we decided not to expose 
three-way diff to reduce operation complexity and confusion for end users.
...
Table \ref{tab:expdiff} shows the time of the built-in diff and SQL diff operations.
Built-in diff operations are much faster than the SQL implementations in all cases.
Built-in diff only need to scan the $\Delta$s and its performance does not depends
the size of the whole table.  Built-in diff operations is just a query and is optmized 
by query optimizer.  The C2 and C3 of NoPK case crossed an threshold that optimizer 
picked up different parallelization strategies.
...
Generating the diff set takes the majority of the time for performing merge.
As expected, Table \ref{tab:expmerge} shows that built-in merge are much 
faster than SQL implementations in all cases.  From the SQL diff and merge 
...
The built-in diff and merge operations all perform better with a primary key
than without.  More efficient lookups is one reason, another reason is that
the scanning and diff aggregation on $\Delta$s can collapsing operations
and skip unnecessary checkings. For example, inserted rows produced 
from scanning $\Delta$ either has a matching deleted row, or it is 
a valid inserted row without conflict.
...
diff aggregation than that in the case without primary key because updates
on the same primary key are collapsed, but this difference is not significant.
