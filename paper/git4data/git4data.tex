...
(and potentially from each other). This indicates that both branches independently 
modified the same row. This is a true conflict. \texttt{SKIP} retains the version 
...
values. This is a true conflict. \texttt{SKIP} mode retains $N_{sn2}$ rows, 
\texttt{ACCEPT} mode retains $N_{sn3}$ rows, and \texttt{FAIL} mode aborts the merge.
\end{enumerate}
