...
\title{MatrixOne: Git For Your Data}
...
engineering.  Git and the git services such as GitHub has become the standard
...
Let's consider table T in a database.  We can refer to a snapshot of T at timestamp
ts as \texttt{T\{mo\_ts = ts\}}.  For example, user can read a table at a specific
timestamp using \texttt{SELECT * FROM T\{mo\_ts = '2025-09-12 12:34:56'\}}.
...
for a table using \texttt{CREATE SNAPSHOT snapshot1 FOR TABLE T} and refer to it
using \texttt{T\{snapshot = 'snapshot1'\}}. User can also take a snapshot of a database,
which is the collection of snapshots of all tables in the database.





\subsection{Clone/Branch}

\subsection{Push/Pull}

\subsection{Diff/Merge}
We will also discuss the challenges and limitations
of the current implementation.  Finally, we will discuss the future directions
of the project.
